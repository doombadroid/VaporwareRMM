# Codex #6 — Agent Re-Registration Hijack (Proposal, Not Merged)

**Status:** Proposal. This branch is intentionally **not** merged. Pick a design and open a real fix PR after review.
...
- Should the existing-token check be **mandatory** for all re-registrations, or only required when the requesting bearer is brand-new? (Recommend: mandatory, no carve-outs. Carve-outs become the exploit.)
- Should `agent_tokens` track the **previous** token hash for a grace window, so a service that re-registers immediately after a token rotation still has a valid PoP value? (Recommend: yes, same 60s window as today's `superseded_at`. Otherwise rapid re-register loops break themselves.)
- What is the response code for the conflict path? 409 (Conflict) is correct, but log carefully: an attacker probing will generate one of these per attempt and operators need an alert hook (suggest: trigger `device.registration_conflict` webhook, audit-log every occurrence).
- Threshold for auto-locking a tenant's `REGISTRATION_SECRET` after N conflicts in M seconds — yes/no? (Recommend: alert, do not auto-lock; auto-lock is its own DoS primitive against any tenant whose secret leaked.)
- Migration path for **already-deployed agents that did not persist their token** (older builds): a one-time admin-acknowledged "trust on first re-register" flag per device, expiring after one use, would let operators carry the fleet across the cutover without a fleet-wide reinstall. Worth the extra complexity? Open question.

## 7. What this branch contains

This document, only. No code changes. The intent is to land the design choice via a separate, scoped PR after reviewer feedback.
