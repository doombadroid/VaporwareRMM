...
> **Command delivery is pull-only.** The agent polls the server every 15 seconds for queued commands; the server does **not** push commands to the agent. When you click "Run command" / "Install Sunshine" / "Enroll Tailscale" in the dashboard, the row is queued and runs at the next poll — **expected p50 latency from click to execution is ~7.5 seconds, p95 ~15 seconds, plus command runtime**. UIs that imply "click → instant" should be calibrated to that. We previously had a server→agent push path but it was structurally broken (the server only ever holds the SHA-256 hash of the agent's bearer token; the agent compares against plaintext); push has been removed and pull is the only delivery path. Real-time push would require either an outbound-only WebSocket initiated by the agent or a separate server-side bearer issued at registration time — both are deliberate roadmap items, not regressions.
